function [out, ly, ny, M] = PetviashviliMethodMk4(Nin, Omega_in, A, k0, nsteps)
...
global k Ak Pk Hk Omega
  ...
  yk0 = yk;
  for j = 1:nsteps
    M = (linear(yk)*yk')/(nonlinear(yk, 1./sqrt(M))*yk');
    
    yk = nonlinear(yk, 1./sqrt(M))./(abs(k)+1);
    %yk = yk/abs(2*yk(end));
    %yk = yk/sqrt(mu);
    %yk = yk/sqrt(mu);
    yk = yk*M;
  ...
  res = linear(yk)-M*nonlinear(yk, 1./sqrt(M));
  ly = linear(yk);
  ny = nonlinear(yk, 1./sqrt(M));

  res2 = (abs(k)+M).*yk/M - nonlinear2(yk,M);

  NormR = sqrt(sum(abs(res).^2));
  NormR2 = sqrt(sum(abs(res2).^2));
  printf('Norm of Residual = %.12e\tOther form = %.12e\n', NormR, NormR2);
  figure(1)
...
function outk = linear(ink)
 ...
 outk = real((abs(k)+1).*ink);
end

function outk = nonlinear(ink, Lambda)
 global k Ak Pk Hk Omega
...
function outk = nonlinear2(ink, MM)
 global k Ak Pk Hk Omega
 ...
 outk = 0.5*fft( real(dz).*tmp - ifft(Hk.*fft(imag(dz).*tmp )))/N; 
 outk = outk + 0.25*MM*Omega^2.*Ak.*fft( real(z).*kAbsZ2 - ifft(Hk.*fft(imag(z).*kAbsZ2)))/N;
 outk = real(outk);
 outk(N/4:3*N/4) = 0; 
 %idk = zeros(1, N); idk(1) = 1;
 %tmp = ratio_fourier( ifftshift(idk), ifftshift(fft(abs(dZ))));
